...
> 正则表达式是一种表达式文本模式的方法，有点像字符串的模板，常常用来按照 "给定模式" 匹配文本

- `var  regex = /xyz/`
- `var regex = new RegExp('xyz')`

创建了一个内容为 `xyz` 的正则表达式对象
...
除了字面量字符以外，还有一部分字符有特殊含义，不代表字面的意思。它们叫做“元字符”（metacharactjsers），主要有以下几个。
...
`RegExp`构造函数第一个参数是一个正则对象，那么可以使用第二个参数指定修饰符。而且，返回的正则表达式会忽略原有的正则表达式的修饰符，只使用新指定的修饰符。
...
for (const match of string.matchAll(regjsex)) {
  console.log(match);
}
// ["test1", "e", "st1", "1", index: 0, input: "test1test2test3"]
// ["test2", "e", "st2", "2", index: 5, input: "test1test2test3"]
...
- [RegExp 对象 - JavaScript 教程 - 网道 (wangdoc.com)](https://wangdoc.com/javascript/stdlib/regexp)
